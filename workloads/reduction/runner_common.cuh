...
#define OPTIMIZER_CUDA_CHECK(expression) \
    ::optimizer_reduction::check_cuda((expression), #expression, __FILE__, __LINE__)
...
int run_workload(const Options& options, const char* expected_variant, Launch launch) {
    ...
    const std::size_t count = static_cast<std::size_t>(options.size);
    ...
    OPTIMIZER_CUDA_CHECK(cudaMalloc(&device_output, sizeof(float)));
    OPTIMIZER_CUDA_CHECK(cudaMemcpy(device_input, host_input.data(), bytes, cudaMemcpyHostToDevice));
    launch(device_input, device_output, options.size, nullptr);
    OPTIMIZER_CUDA_CHECK(cudaDeviceSynchronize());
    float actual = 0.0f;
    OPTIMIZER_CUDA_CHECK(cudaMemcpy(&actual, device_output, sizeof(float), cudaMemcpyDeviceToHost));
    const double absolute_error = std::fabs(static_cast<double>(actual) - expected);
    const bool correct = absolute_error <= 2.0e-2 + 2.0e-5 * std::fabs(expected);
    ...
    if (options.correctness) {
        std::cout << "{\"variant\":\"" << expected_variant << "\",\"size\":" << options.size
                  << ",\"shape\":[" << options.size << "],\"dtype\":\"float32\",\"device\":\""
        ...
        const double bandwidth = static_cast<double>(bytes) / (median * 1.0e6);
        std::cout << "{\"variant\":\"" << expected_variant << "\",\"size\":" << options.size
                  << ",\"shape\":[" << options.size << "],\"dtype\":\"float32\",\"device\":\""
                  << json_escape(properties.name)
                  << "\",\"backend\":\"cuda\",\"hardware_mode\":\"cuda\""
