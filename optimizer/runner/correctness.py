...
def run_correctness(
    ...
    if build_result.get("mode") == "cuda_nvcc":
        ...
        if not executable.is_file():
            return _not_run("NOT_RUN_MISSING_CUDA_EXECUTABLE", variant, build_result)
        command = [str(executable), "--variant", variant, "--size", str(config.input_size), "--mode", "correctness"]
    ...
    if timed_out:
        payload.update({"correctness_pass": False, "status": "CORRECTNESS_TIMEOUT"})
    elif return_code != 0:
        payload.update({"correctness_pass": False, "status": "CORRECTNESS_FAIL"})
    return payload
